<x-layout-admin-base>
        ...
        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
            <div data-kt-place="true" data-kt-place-mode="prepend" data-kt-place-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center me-3 flex-wrap mb-lg-0 mb-sm-0 mb-0 lh-1">
                <h1 class="d-flex align-items-center text-dark fw-bolder my-1 fs-3"><a href="{{ route('admindashboard') }}">Dashboard</a>
                    ...
                    <small class="text-muted fs-7 fw-bold my-1 ms-1">Institute Users</small>
                </h1>
            </div>
        </div>
    </div>
            ...
            <div class="card mb-5 mb-xl-8">
                <div class="card-header border-2 pt-2">
                    <h3 class="card-title align-items-start flex-column">
                        <span class="card-label fw-bolder fs-3 mb-1">Institute Users</span>
                    </h3>
                    <div class="card-toolbar" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-trigger="hover" title="" data-bs-original-title="Click to add a Course Intent">
                        <a href="javascript:void(0)" onclick="CreateInstitute()" class="btn btn-sm btn-primary">
    ...
    <script type="text/javascript">
        ...
        function getAuthor(identifier) {
            var id = $(identifier).data('id');
            $(".loading").show();
            var _url = '{{ route("instituteAuthor", ":id") }}';
            _url = _url.replace(':id', id);
            GetCallAjax(_url);
        }

        function editInstitute(identifier) {
            var id = $(identifier).data('id');
            $(".loading").show();
        ...
        function editInstituteAuthor(identifier){
            var id = $(identifier).data('id');
            $(".loading").show();
            var _url = '{{ route("admin.institute.edit", ":id") }}';
            _url = _url.replace(':id', id);
            GetCallAjax(_url);
        }
        ...
        function deleteUserprofile(delete_id) {
