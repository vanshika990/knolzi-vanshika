        ...
        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
            <div data-kt-place="true" data-kt-place-mode="prepend" data-kt-place-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center me-3 flex-wrap mb-lg-0 mb-sm-0 mb-0 lh-1">
                <h1 class="d-flex align-items-center text-dark fw-bolder my-1 fs-3"><a href="{{ route('admindashboard') }}">Dashboard</a>
                    ...
                    <small class="text-muted fs-7 fw-bold my-1 ms-1">Contact-us page setting</small>
                </h1>
            </div>
        </div>
    </div>
    <form class="kt-form" action="{{ route('home-page-setting-post') }}" name="contactuspagesetting" id="contactuspagesetting" method="POST" enctype="multipart/form-data">
    @csrf
                ...
                <div class="card mb-5 mb-xl-8">
                    <div class="card-header border pt-2">
                        <h3 class="card-title align-items-start flex-column">
                            <span class="card-label fw-bolder fs-3 mb-1">Contact-us hero section</span>
                        </h3>
                    </div>
                    <div class="card-body py-3">
                        <input type="hidden" name="hero_sec_id" value="{{ (empty($data)) ? '' : 'contactuspage_hero_section' }}">
                        <div class="row">
                            <div class="py-3 col-md-6">
                                <label for="hero_sec_title" class="required form-label">Hero Image Title</label>
                                <input type="text" name="hero_sec_title" id="hero_sec_title" class="form-control form-control-solid" placeholder="Enter Title " value="{{ (empty($data)) ? '' : $data['contactuspage_hero_section']['hero_sec_title'] }}" required />
                            </div>
                            <div class="py-3 col-md-6 course-img-prevw">
                                ...
                                <input id="hero_sec_oldimage" type="hidden" name="hero_sec_oldimage" value="{{ (empty($data)) ? '' : $data['contactuspage_hero_section']['hero_sec_image'] }}">
                                @if(!empty($data))
                                    <a href="{{ $data['contactuspage_hero_section']['hero_sec_image'] }}" class="btn m-b-5 mt-2" target="_blank"><i class="bi bi-eye"></i> Preview</a>
                                @endif
                            </div>
                            <div class="py-3 col-md-12">
                                ...
                                <textarea name="hero_sec_description" id="hero_sec_description" rows="10" cols="80" >
                                    @if(!(empty($data)))
                                        {{ $data['contactuspage_hero_section']['hero_sec_description'] }}
                                    @endif
                                </textarea>
                            </div>
                        </div>
        ...
        $(document).ready(function() {
            ...
            $("#contactuspagesetting").validate({
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        url: "{{ route('contact-us-page-setting-post') }}",
                        type: 'POST',
                        contentType: false,
                        data: data,
                        processData: false,
