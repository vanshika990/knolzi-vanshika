<x-layout-admin-base>
            ...
            <div class="card mb-5 mb-xl-8">
                <div class="card-header border-2 pt-2">
                    <h3 class="card-title align-items-start flex-column">
                        <span class="card-label fw-bolder fs-3 mb-1">Coupon</span>
                    </h3>
                    <div class="card-toolbar" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-trigger="hover" title="" data-bs-original-title="Click to add a Coupon">
    ...
    <script type="text/javascript">
        $(document).on("click", '#status_changed', function() {
            var coupon_id = $(this).attr("uid");
            var status = $(this).text();
            var titleText = 'Are you sure you want to active this Coupon ?';
            if (status == 'Active') {
                var titleText = 'Are you sure you want to inactive this Coupon ?';
            }
            swal({
                title: titleText,
                type: "warning",
                ...
                if (isConfirm) {
                    ...
                    PostPutAjaxCall('{{ route("coursechangestatus") }}', 'POST', data);
                } else {
        ...
        function Viewdetails(coupon_id) {
            ...
            $(".loading").show();
            var _url = '{{ route("admin.coupon.show", ":id") }}';
            _url = _url.replace(':id', id);
            GetCallAjax(_url);
        }

        function ViewCouponused(identifier){
            var id = $(identifier).data('id');
            $(".loading").show();
            var _url = '{{ route("coupon-used-user",":id") }}';
            _url = _url.replace(':id', id);
            GetCallAjax(_url);
        }
        ...
        function deleteCoupon(delete_id) {
            ...
            swal({
                ...
                if (isConfirm) {
                    var _url = '{{ route("admin.coupon.destroy", ":id") }}';
                    _url = _url.replace(':id', id);
                    DeleteAjaxCall(_url);
                } else {
                    swal("Cancelled", "cancelled.", "error");
