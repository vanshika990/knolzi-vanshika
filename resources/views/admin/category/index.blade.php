<x-layout-admin-base>
        ...
        <div id="kt_content_container" class="container">
            <div class="card mb-5 mb-xl-8">
                <div class="card-header border-2 pt-2">
                    ...
                    <div class="card-toolbar" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-trigger="hover" title="" data-bs-original-title="Click to add a Category">
                        <a href="javascript:void(0)" onclick="CreateCategory()" class="btn btn-sm btn-primary">
    ...
    <script type="text/javascript">
        $(document).on("click", '#status_changed', function() {
            ...
            var titleText = 'Are you sure you want to active this Category ?';
            if (status == 'Active') {
                var titleText = 'Are you sure you want to deactive this Category ?';
            }
            swal({
                title: titleText,
                type: "warning",
                ...
                if (isConfirm) {
                    ...
                    PostPutAjaxCall('{{ route("categorychangestatus") }}', 'POST', data);
                } else {
        ...
        function CreateCategory() {
            GetCallAjax("{{ route('admin.category.create') }}");
        }
        function editCategory(edit_id) {
            var id = $(edit_id).data('id');
            $(".loading").show();
            var _url = '{{ route("admin.category.edit", ":id") }}';
            _url = _url.replace(':id', id);
            GetCallAjax(_url);
        }
        function deleteCategory(delete_id) {
            ...
            swal({
                ...
                if (isConfirm) {
                    var _url = '{{ route("admin.category.destroy", ":id") }}';
                    _url = _url.replace(':id', id);
                    DeleteAjaxCall(_url);
                } else {
                    swal("Cancelled", "cancelled.", "error");
