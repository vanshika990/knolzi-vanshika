@if (!empty($question))
    ...
    <script>
        $(document).ready(function() {
            ...
            var imgTimer = null,
                imgValue = 0;
            var helpVideoTimer = null,
                helpVideoValue = 0;
            var helpAudioTimer = null,
                helpAudioValue = 0;
            var helpPdfTimer = null,
                helpPdfValue = 0;
            var helpImgTimer = null,
                helpImgValue = 0;
            ...
            $(".next-que").click(function() {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data: {
                        course_id: "{{ encrypt($question['course_id']) }}"
                    },
                    ...
                    if (xhr.status == 422) {
                        if (xhr.responseJSON.errors) {
                            $.each(xhr.responseJSON.errors, function(i, val) {
                                errors += "<b><p style='color:#F44336'>" + val[0] +
                                    "</p></b><br/>";
                            });
                            if (errors !== "") {
                                swal({
            ...
            $("#submitquestion").validate({
                ...
                submitHandler: function(form) {
                    ...
                    data.append('choice', parseInt($('input[name=answer]:checked').attr("data-key")) +
                        1);
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            $("#CourseComment").html(response.html);
                            $('#CourseAddNewCommentModal').modal('hide');
                            $(".modal-backdrop").remove();
                            form.reset();
                        }
                    }).fail(function(xhr, textStatus, errorThrown) {
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            $("#accordionQA").html(response.html);
                            $('#accordionQAModal').modal('hide');
                            $(".modal-backdrop").remove();
                            form.reset();
                        }
                    }).fail(function(xhr, textStatus, errorThrown) {
